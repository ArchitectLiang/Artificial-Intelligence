    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                for i in range(4):
                    if self.corners[i] == nextPosition:
                        temp = list(state[1])
                        temp[i] = True
                        nextState = (nextPosition, tuple(temp))
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(state[1])):
        if state[1][i] == False:
            maze_distance = mazeDistance(state[0], corners[i], problem.startingGameState)
            hn = max(maze_distance, hn)
        ...
        for j in range(foodGrid.height):
            if not problem.walls[i][j]:
                if foodGrid[i][j] == True:
                    maze_distance = mazeDistance(position, (i,j), problem.startingGameState)
                    hn = max(maze_distance, hn)
    return hn
