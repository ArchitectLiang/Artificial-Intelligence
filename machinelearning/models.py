...
class RegressionModel(object):
    ...
    def __init__(self):
        ...
        self.w1 = nn.Parameter(self.feature_dimension, self.hidden_layer_size)
    ...
    def run(self, x):
        ...
        xw2 = nn.Linear(nn.ReLU(xw1_b1), self.w2)
        xw2_b2 = nn.AddBias(xw2, self.b2)
        return xw2_b2
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w2 = nn.Parameter(self.hidden_layer_size, 5)
    ...
    def run(self, xs):
        ...
        for i in range(len(xs) - 1):
            z = nn.Add(nn.Linear(xs[1+i], self.w1), nn.ReLU(nn.Linear(z, self.wh)))
            z = nn.AddBias(z, self.b1)
        z = nn.Linear(z, self.w2)
        z = nn.AddBias(z, self.b2)
        return z
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(self.batch_size):
            loss = self.get_loss(x, y)
            grad_wrt_w1, grad_wrt_b1, grad_wrt_wh, grad_wrt_w2, grad_wrt_b2 = nn.gradients(loss, [self.w1, self.b1, self.wh, self.w2, self.b2])
            self.w1.update(grad_wrt_w1, -self.alpha)
            self.b1.update(grad_wrt_b1, -self.alpha)
            self.wh.update(grad_wrt_wh, -self.alpha)
            self.w2.update(grad_wrt_w2, -self.alpha)
            self.b2.update(grad_wrt_b2, -self.alpha)
            counter += 1
            if (counter % 5 == 0):
