...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in livingGhostPositionDistributions:
            mostlikelypos += [i.argMax()]
        closestghost = mostlikelypos[0]
        distance = self.distancer.getDistance(pacmanPosition, closestghost)
        for i in mostlikelypos:
            if self.distancer.getDistance(i, pacmanPosition) < distance:
                closestghost = i
                distance = self.distancer.getDistance(i, pacmanPosition)
        result = legal[0]
        successorPosition = Actions.getSuccessor(pacmanPosition, result)
        dist = self.distancer.getDistance(successorPosition, closestghost)
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(successorPosition, closestghost) < dist:
                result = action
                dist = self.distancer.getDistance(successorPosition, closestghost)
        return result
